...
PP.data.wide[is.na(PP.data.wide)] <- 0
...
model.data.col <- model.data[,PredNames] 
...
for (i in PredNames){
  Tol.DF <- data.frame(Pred = model.data.col[,i], 
                       model.data.col[,PredNames[!PredNames%in%i]])
  Tol.LM <- lm(Pred~.,
               data = Tol.DF) 
...
#Ingen af værdierne er over 10. Derfor beholder vi alle variablene
